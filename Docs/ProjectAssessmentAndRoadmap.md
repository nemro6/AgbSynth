...
1. 編集状態を確実に保存する仕組みが未完成で、Save、Dirty管理、Undo/Redoが機能していない。
...
| Save Project | 未実装 | ツールバーにボタンはあるが処理が接続されていない。 |
| Undo/Redo | 未実装 | ボタンは常に無効。 |
...
### P0-1. Saveと編集トランザクションがない

ツールバーのSave Projectボタンには処理が接続されていません。Undo/Redoも未実装です。新規資産の作成時にはファイルを書きますが、既存資産の編集はメモリ上だけの箇所があり、保存方法が資産ごとに統一されていません。

この状態では、次の事故が起こり得ます。

- 編集後に別プロジェクトを開いて変更を失う
- Refreshで未保存状態をディスク内容に戻す
- 一部の資産だけ保存され、リンク先と内容が不整合になる
- 削除した資産を参照するファイルが残る
- 保存途中の異常終了でJSONを破損する
