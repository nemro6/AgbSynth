...
進捗（2026-07-12）: Stopwatch schedulerを廃止し、Emeraldの`MPlayMain`と同じVBlank/整数`tempoC`駆動へ移行。コア実装と自動テストは完了し、代表ROMの聴感・イベント比較が残っている。

目的: 現行挙動の維持ではなく、GBA MP2Kの更新順と整数タイミングでMIDIイベントを安定して投入する。

実装内容:

- `m4aSoundMain`の59.7275 Hz VBlank更新と`tempoC += tempoI`、閾値150の整数clockを実装
- 初回イベントを次のVBlank境界へ揃え、Tempo変更後も整数剰余を保持
- MP2K tick、同一tick内のイベント順、トラック別loopを`Mp2kMidiPlaybackSession`へ集約
- AudioEngine内でVBlank境界からsessionを進め、UI timerとOS schedulerへの依存を除去
- 再生用channel state、音色解決、発音、CC処理をUI ViewModelから分離
- 使用音色、PCM、Wave RAMを再生前に先読みし、audio callback中のファイルI/Oを除去
- UIはimmutable snapshotだけを読み、audio threadからAvalonia bindingを更新しない
- 75/150/50 Tempo、pause、トラック別loop、32768 Hz時のVBlank境界を回帰テスト化
- realtime `Read`と手動/offline `Read`が同じsession clockを利用

完了条件:

- UI負荷を掛けてもイベントのVBlank位置が変わらない
- Emeraldの`MPlayMain`と初回tick、Tempo変更、整数剰余、track loop位置が一致する
- PSGとPCMを同じMPlay tickで投入し、片方だけ先行させない
- 同じ入力と設定の手動/offline `Read`が毎回同一になる
...
1. Git管理、構造整理、Save/Dirty/Undo/Redoの基盤を維持する。
2. Phase 3のVBlank/`tempoC` schedulerをFireRed/Emeraldの代表曲で確認する。
3. 問題があればStopwatchへ戻さず、MP2Kの更新順との差分をfixtureで修正する。
4. 保留したPhase 2へ戻り、AssetId、Rename、依存検証、Diagnosticsを導入する。
5. SongHeader単位のMIDI/Midi2agb切替と相互Import/Exportを実装する。
6. Compiler/LinkerとAuto/Manual配置ウィンドウを完成させる。
7. 実機比較、UI共通化、特殊PCM、Cry Editor、配布機能へ進む。

Phase 3は現行Stopwatch再生の維持ではなく、Emerald MP2KのVBlank駆動を正とします。代表曲で問題が出た場合も壁時計へ戻さず、イベント順、Tempo command位置、track loop、PSG/PCM発音開始のどこが実機と違うかを固定テストへ追加します。
